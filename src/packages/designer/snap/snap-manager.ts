...
export class SnapManager {
  constructor(public toggles: SnapToggles = {
    toElements: true,
    toCanvas: true,
    toGuides: true,
    toGrid: false,
  ...
  snap(
    ...
    if (sides.centerX)
      xSources.push({ key: 'centerX', pos: movingRect.x + movingRect.width / 2 });
    ...
    if (sides.centerY)
      ySources.push({ key: 'centerY', pos: movingRect.y + movingRect.height / 2 });
  ...
  private collectCandidates(ctx: SnapContext, scale: number): {
    x: SnapCandidate[];
    y: SnapCandidate[];
  } {
